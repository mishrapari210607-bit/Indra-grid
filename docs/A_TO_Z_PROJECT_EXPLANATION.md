...
4. `dashboard/app.py` handles login/register and launches the Streamlit dashboard.
5. `dashboard/dashboard.py` renders the enterprise dashboard, charts, reports, recommendations, forecasts, admin view, and account view.
...
- `users`: lists users for the admin dashboard.
- `save_run`: persists a simulation summary to SQLite.
- `runs`: returns recent saved simulations.
...
- Require JWT authentication on `/users` and `/runs`, and restrict admin-only APIs server-side.
- Add a true IoT ingestion endpoint for live meter readings.
- Add ML forecasting for tomorrow's demand and solar instead of seed-shifted simulation.
- Add economic metrics: payback period, battery degradation cost, demand-charge reduction, and ROI.
- Add a one-click demo mode with scripted outage and peak-tariff events.
- Replace hardcoded fault data with database-backed fault events.
